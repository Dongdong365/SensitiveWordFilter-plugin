...
2. 配置项说明
   ...
   - exemptPlayers: 豁免列表，可填玩家名或 UUID
   - thresholds: 累进惩罚规则
       count: 累计违禁次数达到该值时触发
...
3. 控制台指令（仅服务端/控制台可用，无需 - 前缀）
   swf reload                    重载配置与词库
   swf status                    查看敏感词数量与记录玩家数
   swf test <消息>               测试消息是否命中敏感词
   swf add <敏感词>              添加敏感词
   swf remove <敏感词>           移除敏感词
   swf clear <玩家名或UUID>      清除某玩家的违禁记录并解封
   swf clearall                  清除所有玩家违禁记录并解封
   swf info <玩家名或UUID>       查看某玩家违禁次数与封禁状态
   swf unban <玩家名或UUID>      解封某玩家
